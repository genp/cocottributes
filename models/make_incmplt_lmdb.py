...
stmt = "select * from (select patch_id from (select a.patch_id, count(*) from annotation a, label lbl where a.label_id = lbl.id and lbl.parent_id = 407 group by a.patch_id) as tmp where count > 20 ) as foo intersect select p.id from patch p, image im where p.image_id = im.id and im.type = 'train2014'"
patch_ids_train = [x[0] for x in db.engine.execute(stmt).fetchall()]
stmt = "select * from (select patch_id from (select a.patch_id, count(*) from annotation a, label lbl where a.label_id = lbl.id and lbl.parent_id = 407 group by a.patch_id) as tmp where count > 20 ) as foo intersect select p.id from patch p, image im where p.image_id = im.id and im.type = 'val2014'"
patch_ids_val = [x[0] for x in db.engine.execute(stmt).fetchall()]
...
save_path = '/data/hays_lab/COCO/caffemodels_incmpltlbls'
...
with in_db.begin(write=True) as in_txn:
    for in_idx, in_ in enumerate(patch_ids_train):
        # load labels:
        im = np.array(get_multilabel(in_, obj_lbls, parent_lbls, False))# or load whatever ndarray you need
