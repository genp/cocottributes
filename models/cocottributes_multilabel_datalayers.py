...
class CocottributesMultilabelDataLayerSync(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        assert 'split' in params.keys(), 'Params must include split (train2014, val2014, or test).'
        assert 'im_shape' in params.keys(), 'Params must include im_shape.'
        assert 'label_ids' in params.keys(), 'Params must include Cocottributes database label ids.'
        assert 'exhaustivelbls' in params.keys(), 'Params must include whether to use exhaustive labels only or not'
        
        # store input as class variables
        self.batch_size = params['batch_size'] 
        self.im_shape = params['im_shape']
        # TODO: change to use cocottributes dataset indices
        self.label_ids = params['label_ids']
        self.exhaustive = params['exhaustivelbls']
        if self.exhaustive:
            self.indexlist = manage_hits.find_patches(self.label_ids, [], [], splittype = params['split'])
        else:
            self.indexlist = manage_hits.find_patches([], [], self.label_ids, splittype = params['split'])
        print 'Dataset size:{}'.format(len(self.indexlist))
        ...
        top[1].reshape(self.batch_size, len(self.label_ids))
    ...
    def forward(self, bottom, top):
        ...
        for itt in range(self.batch_size):
            ...
            im, multilabel = load_cocottributes_annotation(index, self.im_shape, self.label_ids)
...
class BatchAdvancer():
    ...
    def __call__(self):
        ...
        for itt in range(self.batch_size):
            ...
            # Load an image and prepare ground truth
            index = self.indexlist[self._cur] # Get the image index
            im, multilabel = load_cocottributes_annotation(index, self.im_size)
