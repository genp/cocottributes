...
workdir = '/data/gen_data/COCO/caffemodels_exhaustive_split/finetune_model_adagrad/'
os.chdir(workdir)
...
def check_accuracy(net, num_batches, batch_size = batch_size):
    ...
    for t in range(num_batches):
        ...
        for gt, est in zip(gts, ests): #for each ground truth and estimated label vector
            # hdist = hamming_distance(gt, est)
            hdist = scipy.spatial.distance.hamming(gt, est)
            acc += hdist
...
if not osp.exists(osp.join(workdir, 'figs')):
    os.makedirs(osp.join(workdir, 'figs'))

cur_ind = 0
    
for image_index in range(cur_ind, cur_ind+1):
    ...
    plt.imshow(transformer.deprocess(copy(test_net.blobs['images'].data[image_index, ...])))
