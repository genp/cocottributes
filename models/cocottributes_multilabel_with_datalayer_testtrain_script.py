...
workdir = './cocottributes_multilabel_with_datalayer_finetune'
...
label_ids = [x.id for x in Label.query.filter(Label.parent_id == obj_attr_supercategory_id).order_by(Label.id).all()][:20]#TODO this is a dim test
...
def check_baseline_ap(net, num_batches, num_labels, batch_size = 128):
...
baseline_ap = check_baseline_ap(solver.test_nets[0], 5, num_labels)
ap = check_ap(solver.test_nets[0], 5)
ap_scores = {}
ap_scores['ap'] = ap
